...
import { uploadImage, deleteImage } from "../services/productoService";
...
export function ImageUploader({ urls, onChange }: ImageUploaderProps) {
  ...
  // Extract public_id from a Cloudinary URL for delete calls (best-effort)
  ...
  async function handleRemove(url: string, index: number) {
    ...
    if (publicId) {
      // best-effort — don't block UI if delete fails
